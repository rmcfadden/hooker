...
export function isoToMicros(iso: string): number {
...
export function dayStartMicros(dateStr: string): number {
 ...
 */
export function boundMicros(str: string, { end = false }: { end?: boolean } = {}): number {
 ...
 */
export function durationMicros(str: string): number {
  const match = DURATION.exec(String(str).trim().toLowerCase());
  const unit = match ? UNIT_MICROS[match[2] ?? ""] : undefined;
  ...
  const count = match[1] ?? "";
  return (count === "" ? 1 : Number(count)) * unit;
}
